...
namespace Sitecore.Demo.Platform.Feature.Demo.Controllers
{
	...
	public class DemoController : SitecoreController
	{
		...
		public ActionResult ExperienceData()
		{
			...
			DemoControls demoControls = new DemoControls();
			DemoControls(demoControls);
			experienceData.DemoControls = demoControls;
		...
		private void DemoControls(DemoControls demoControls)
		{
			HttpCookie checkCookie = Request.Cookies[Constants.TogglePersonalizationCookie];
			if (checkCookie != null)
			{
				demoControls.TogglePersonalization = System.Convert.ToBoolean(Request.Cookies[Constants.TogglePersonalizationCookie].Value);
			}
			else
			{
				HttpCookie cookie = new HttpCookie(Constants.TogglePersonalizationCookie, demoControls.TogglePersonalization.ToString());
				cookie.Expires = DateTime.Now.AddDays(1);
				Response.Cookies.Add(cookie);
			}
		}
		public void TogglePersonalization(DemoControls demoControls)
		{
			HttpCookie checkCookie = Request.Cookies[Constants.TogglePersonalizationCookie];
			if (checkCookie != null)
			{
				Response.Cookies[Constants.TogglePersonalizationCookie].Value = demoControls.TogglePersonalization.ToString();
			}
			else
			{
				HttpCookie cookie = new HttpCookie(Constants.TogglePersonalizationCookie, demoControls.TogglePersonalization.ToString());
