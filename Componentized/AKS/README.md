...
```text
kubectl apply -f .\helm-rbac.yaml --validate=false
helm init --service-account tiller --node-selectors "beta.kubernetes.io/os=linux"
```

## Deploy clean Sitecore XM demo

```text
kubectl create namespace xmdemo-master

kubectl apply -f .\xmdemo\sql.yaml -f .\xmdemo\solr.yaml -f .\xmdemo\cm.yaml -f .\xmdemo\cd.yaml --validate=false --namespace xmdemo-master
```
...
kubectl apply -f .\habitat\sql.yaml -f .\habitat\solr.yaml -f .\habitat\cm.yaml -f .\habitat\cd.yaml -f .\habitat\xconnect.yaml -f .\habitat\xconnect-automationengine.yaml -f .\habitat\xconnect-indexworker.yaml -f .\habitat\ingress.yaml --validate=false --namespace sitecore-habitathome-platform
```

## Notes

- NOT working on windows (socat missing): `kubectl port-forward cm-5d474455f6-s4bhj 9999:80 -n sitecoredemo-master`
